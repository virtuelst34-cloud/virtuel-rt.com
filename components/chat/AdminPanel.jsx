...
import { X, ShieldAlert, LayoutDashboard, Users, Gavel, Ban, CheckCircle, VolumeX, Volume2, Trash2, Search, Plus, DoorOpen, BarChart2, Circle, Diamond } from 'lucide-react';
import { SALONS, SALON_TYPES, SALON_EMOJIS_LIST } from '@/lib/chatConfig';
import { DIAMOND_BADGES_DEFAULT, getMergedBadges } from '@/lib/diamondBadges';
...
export default function AdminPanel() {
  ...
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-[2000]">
      <div className="bg-card border border-red-500/30 rounded-2xl w-[780px] max-w-[95vw] max-h-[88vh] flex flex-col overflow-hidden shadow-[0_24px_80px_rgba(0,0,0,0.6)]">
...
function StatCard({ value, label, color }) {
  const colors = { red: 'text-red-400', green: 'text-emerald-400', yellow: 'text-amber-400', blue: 'text-blue-400', purple: 'text-purple-400' };
  return (
    <div className="bg-secondary border border-border rounded-xl p-4 text-center">
      ...
      <div className="text-[10px] text-muted-foreground/50 mt-1 uppercase tracking-widest">{label}</div>
    </div>
  );
}
...
function StatsSection() {
  ...
  return (
    <div>
      <SectionTitle icon={BarChart2}>Statistiques</SectionTitle>
      <div className="grid grid-cols-3 gap-2.5 mb-6">
        <StatCard value={totalMessages}  label="Messages total"   color="blue" />
        <StatCard value={avgLevel}       label="Niveau moyen"     color="purple" />
        <StatCard value={premiumCount}   label="Membres premium"  color="yellow" />
      </div>
