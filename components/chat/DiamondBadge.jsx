...
const DiamondBadge = memo(function DiamondBadge(props) {
  const { level, size = 'sm', showLabel = false } = props;
  const { customBadges } = useBadges() || {};
  const badge = getBadgeForLevel(level, customBadges || []);

  const pxMap = { xs: 16, sm: 20, md: 28, lg: 36 };
  const px = pxMap[size] || pxMap.sm;
  const showPing = size === 'md' || size === 'lg';

  return (
    <span className="relative group inline-flex items-center gap-1 shrink-0" title={badge.label}>
