...
function JoinToast({ name, onDone }) {
...
export default function ChatArea({ micActive, micLevel, onOpenDM }) {
  ...
  useEffect(() => {
    ...
    if (salon.welcome) {
      ...
      const already = getMessages(currentSalon).some(m => m.id === welcomeId);
      if (!already) {
  ...
  const handleSend = useCallback((text, imageUrl, reply = null) => {
    ...
    const newMsg = {
      ...
      image_url: imageUrl || '',
      reactions: {},
      replyTo: reply ? { id: reply.id, author_name: reply.author_name, text: reply.text } : null,
      created_date: new Date().toISOString(),
    };
  ...
  const handleReact = useCallback((msgId, event, emoji) => {
    if (emoji) {
      const msgs = getMessages(currentSalon);
      ...
      updateReaction(currentSalon, msgId, reactions);
    } else if (event) {
      setReactionPicker({ msgId, x: event.clientX, y: event.clientY });
    }
  }, [currentSalon, getMessages, updateReaction, user]);

  const handleDelete      = useCallback((msgId) => deleteMessage(currentSalon, msgId), [currentSalon, deleteMessage]);
  const handlePin         = useCallback((msgId) => pinMessage(currentSalon, msgId), [currentSalon, pinMessage]);
  const handleViewProfile = useCallback((name) => { if (name !== user?.name) setViewProfile(name); }, [user]);
            ...
            <div className="flex-1 overflow-y-auto py-1">
              {allMembers.map((m, i) => (
                <button key={m.name + i} onClick={() => handleViewProfile(m.name)}
                  ...
                  {m.speaking && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />}
                </button>
              ))}
            </div>
          </div>
