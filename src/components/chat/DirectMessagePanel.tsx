...
export default function DirectMessagePanel({ onClose, initialUser }: DirectMessagePanelProps) {
  ...
  const contact  = selectedUser ? (profiles[selectedUser] || { name: selectedUser, avatar: 'av1', initials: selectedUser.slice(0,2).toUpperCase(), level: 1, status: 'offline' as const }) : null;
  ...
  const sendPushNotification = useCallback((fromName: string, msg: string) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[1500] animate-in fade-in duration-300 p-4" onClick={onClose}>
      <div className="bg-card border border-border/50 rounded-3xl w-full max-w-[720px] h-[90vh] max-h-[560px] flex overflow-hidden shadow-[0_32px_96px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-300"
        ...
        <div className="w-[210px] bg-secondary border-r border-border flex flex-col shrink-0">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between">
            ...
            <button onClick={onClose} className="p-1 rounded-lg text-muted-foreground/50 hover:text-foreground hover:bg-white/5 transition-all duration-200 hover:scale-110 active:scale-95">
