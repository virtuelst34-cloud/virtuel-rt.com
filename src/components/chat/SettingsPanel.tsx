...
import { X, User, Palette, Shield, Check, Edit3, Sun, Moon, Flame, Calendar, UserX, Star, PartyPopper, Diamond, Minimize2, LucideIcon } from 'lucide-react';
...
export default function SettingsPanel({ onClose }: SettingsPanelProps) {
  ...
  useEffect(() => {
    setDraft({ bio: user?.bio || '', avatar: user?.avatar || 'av1', statusText: user?.statusText || '' });
  }, [user]);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[2000] animate-in fade-in duration-300 p-4" onClick={onClose}>
      <div className="bg-card border border-border/50 rounded-3xl w-full max-w-[580px] max-h-[90vh] flex flex-col overflow-hidden shadow-[0_32px_96px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-300"
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {!editing
                    ...
                    : <div className="flex gap-2">
                        <button onClick={() => { setDraft({ bio: user.bio||'', avatar: user.avatar||'av1', statusText: user.statusText||'' }); setEditing(false); }} className="px-3 py-1.5 rounded-lg bg-white/5 border border-border text-muted-foreground text-xs hover:bg-white/10 transition-colors">Annuler</button>
