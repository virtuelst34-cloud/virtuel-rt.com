...
export default function WelcomeScreen({
  ...
  const setFeaturedSalonRemote = useCallback(async (salonId: string | null) => {
    try {
      ...
      addNotification({ type: 'system', message: 'Impossible dՎpingler le salon (droits staff ?)' });
    }
  }, [refreshSettings, addNotification]);

  const activeSalonsCount = useMemo(
    () => Object.values(salonCounts).filter((n) => n > 0).length,
    [salonCounts],
  );
