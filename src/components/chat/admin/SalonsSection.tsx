...
export default function SalonsSection() {
  ...
  const SALON_EMOJI_MAP: Record<string, string> = {'musique60':'🎵','musique80':'🎸','karaoke':'🎤','debat':'⚡','quiz':'🧠','jeunes':'👋','lgbt':'🌈','divorce':'💙','libre':'🚪','insulte':'😤','cameras':'📹','bar':'🍷'};
      ...
      <div className="space-y-1.5 mb-5">
        {customSalons?.map(s => (
          <div key={s.id} className="flex items-center gap-2.5 bg-secondary border border-border rounded-xl px-3 py-2">
            ...
            <button onClick={() => deleteSalon(s.id)} className="text-muted-foreground/40 hover:text-red-400 transition-colors">
            ...
            {hiddenSalons.map(id => {
              ...
              return (
                <div key={id} className="flex items-center gap-2.5 bg-secondary/50 border border-border/50 rounded-xl px-3 py-2">
                  ...
                  <button onClick={() => handleRestoreSalon(id)} className="text-xs text-emerald-400 hover:text-emerald-300 transition-colors">
                    Restaurer
                  </button>
                </div>
              );
