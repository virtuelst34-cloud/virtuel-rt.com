          ...
          <div key={profile.name} className={`rounded-xl border px-3 py-2.5 ${profile.isBanned ? 'bg-red-500/5 border-red-500/25' : profile.isMuted ? 'bg-amber-500/5 border-amber-500/20' : 'bg-secondary border-border'}`}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex items-center gap-1 shrink-0">
                {!profile.isBanned
                  ...
                  : <button onClick={() => !readOnly && unbanUser(profile.name)}
                      disabled={readOnly} title="Débannir"
                      className={btnClass('bg-emerald-500/15 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/25')}>
                      <CheckCircle className="w-3 h-3" />
                    </button>
                }
                {!profile.isMuted
                  ? <button onClick={() => !readOnly && muteUser(profile.name)}
                      disabled={readOnly} title="Muter"
                      className={btnClass('bg-amber-500/15 border-amber-500/30 text-amber-400 hover:bg-amber-500/25')}>
                      <VolumeX className="w-3 h-3" />
                    </button>
                  : <button onClick={() => !readOnly && unmuteUser(profile.name)}
                      disabled={readOnly} title="Démuter"
                ...
                <button
                  onClick={() => {
                    ...
                    setProfiles(p => { const n = { ...p }; delete n[profile.name]; return n; });
                  }}
                  disabled={readOnly}
                  className={btnClass('bg-white/5 border-white/10 text-muted-foreground/60 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/30')}
                >
            ...
            <div className="flex items-center gap-1.5 mb-2">
              ...
              {STATUS_OPTIONS.map(s => (
                <button key={s.id}
                  onClick={() => !readOnly && setUserStatusAdmin(profile.name, s.id as any)}
                  disabled={readOnly}
                  title={s.label}
                  className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] border transition-all disabled:cursor-not-allowed ${
                    (profile.status || 'online') === s.id
