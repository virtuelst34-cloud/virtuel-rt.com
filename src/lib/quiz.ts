...
class QuizService {
  ...
  submitAnswer(
    ...
    questionId: string,
    answer: number
  ): { isCorrect: boolean; pointsEarned: number; isLate: boolean } {
    const quiz = this.activeQuizzes.get(quizId);
    if (!quiz || !quiz.isActive) {
      return { isCorrect: false, pointsEarned: 0, isLate: true };
    }

    const question = quiz.questions.find(q => q.id === questionId);
    if (!question) {
      return { isCorrect: false, pointsEarned: 0, isLate: false };
    }
    ...
    return { isCorrect, pointsEarned, isLate: false };
  }
